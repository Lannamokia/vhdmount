...
VHD Mounter现在支持作为Windows服务运行，具有管理员权限，可以实现开机自动启动服务，服务启动时自动拉起窗口界面。
...
```cmd
# 安装服务（开机自动启动）
sc create VHDMounterService binPath= "C:\Path\To\VHDMounter.exe --service" start= auto DisplayName= "VHD Mounter Service"
...
- **开机自动启动**：服务在系统启动时自动运行，无需用户登录
- **自动拉起界面**：服务启动时自动拉起WPF窗口界面
