...
test('GET /api/machines/:machineId/deployments/pending 需要机台签名认证', async (t) => {
    const { app } = await createInitializedHarness(t);
    const res = await request(app).get('/api/machines/machine-001/deployments/pending');
    assert.strictEqual(res.status, 401);
});

test('GET /api/machines/:machineId/deployments/pending 已审批签名机台可读取空任务列表', async (t) => {
    const { app, testKeyPair, testKeyId } = await createInitializedHarness(t);
    const path = '/api/machines/machine-001/deployments/pending';
...
test('GET /api/machines/:machineId/deployments/pending 未审批机台会被拒绝', async (t) => {
    const { app, database, testKeyPair, testKeyId } = await createInitializedHarness(t);
    const machine = await database.getMachine('machine-001');
...
test('GET /api/machines/:machineId/deployments/pending 已吊销机台会被拒绝', async (t) => {
    const { app, database, testKeyPair, testKeyId } = await createInitializedHarness(t);
    const machine = await database.getMachine('machine-001');
...
test('POST /api/machines/:machineId/deployments/:taskId/status 需要机台签名认证', async (t) => {
...
test('POST /api/machines/:machineId/deployments/:taskId/status 需要校验任务归属机台', async (t) => {
    const { app, database, testKeyPair } = await createInitializedHarness(t);
    const otherKeyPair = crypto.generateKeyPairSync('rsa', { modulusLength: 2048 });
    const { keyId: otherKeyId } = await registerMachine(database, 'machine-002', otherKeyPair, {
        keyId: 'test-key-002',
        approved: true,
        revoked: false,
    });
...
test('POST /api/machines/:machineId/deployments/:taskId/status 允许签名机台回写自己的任务', async (t) => {
    const { app, database, testKeyPair, testKeyId } = await createInitializedHarness(t);
...
test('POST /api/machines/:machineId/deployments/sync 允许签名机台同步自己的记录', async (t) => {
    const { app, testKeyPair, testKeyId } = await createInitializedHarness(t);
